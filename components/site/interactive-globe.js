...
export default function InteractiveGlobe() {
  ...
  useEffect(() => {
    ...
    fetch('https://raw.githubusercontent.com/vasturiano/react-globe.gl/master/example/datasets/ne_110m_admin_0_countries.geojson')
      .then((res) => (res.ok ? res.json() : Promise.reject('Failed to load geometry')))
      .then((data) => {
        if (active) setCountries(data.features || []);
      })
      .catch(() => {
        // Fallback: silent degradation if offline or CDN unavailable
      });
  ...
  const handleGlobeReady = () => {
    ...
    try {
      ...
      const animateRings = () => {
        animRef.current = requestAnimationFrame(animateRings);
        if (ring1) {
      ...
      // Catch any canvas initialization glitches silently
    }
  };
